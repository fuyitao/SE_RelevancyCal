...
int main(){
	...
	string weightFile        = path + "relevancyCalculate_dict/xihuan_bm25f_new";      // relevancy
	//---------------------------------
	...
	CalculateRelevancy calRelevancy(weightFile);  // relevancy
	gettimeofday(&tv, NULL);
	ends = tv.tv_sec*1000 + tv.tv_usec/1000;
	...
	while (1){
		...
		calRelevancy.GetScoreList(termStruct.termStr, idVec, relStruct, topN);
		// output
